...
train <- window(m1.ts, start=c(1959, 1), end=c(1999, 12), freq=12) # 480 obs
test <- window(m1.ts, start=c(2000, 1), end=c(2001, 12), freq=12) # 24 obs
  # Plot training set
...
train.diff2 <- diff(train.bc, lag=1, differences=2)
...
## Model Selection
  # ACF and PACFS
par(mfrow=c(1, 2))
acf(m1, lag.max=12) # To lag 12
pacf(m1, lag.max=12)
par(mfrow=c(1, 2))
acf(m1, lag.max=60) # To lag 60
pacf(m1, lag.max=60)

  # AICc Selection
    # Model 1
arima(train.bc, order=c(8, 2, 4), seasonal=list(order=c(1, 0, 1), 
      period=12), method="ML")
arima(train.bc, order=c(3, 2, 3), seasonal=list(order=c(1, 0, 1), 
      period=12), fixed=c(NA, NA, NA, 0, 0, NA, NA, NA), method="ML")
    # Model 2
arima(train.bc, order=c(8, 2, 4), 
      method="ML")
arima(train.bc, order=c(7, 2, 4), 
      fixed=c(NA, NA, NA, 0, 0, 0, NA, NA, NA, 0, NA), method="ML")
    # Model 3
arima(train.bc, order=c(2, 2, 4), seasonal=list(order=c(1, 0, 1), 
      period=12), method="ML")
arima(train.bc, order=c(2, 2, 4), seasonal=list(order=c(1, 0, 1), 
      period=12), fixed=c(NA, NA, NA, 0, NA, NA, NA, NA), method="ML")

  # Plotting roots relative to the unit circle
    # Model 1
par(mfrow=c(2, 2))
uc.check(pol_=c(1, 0.6377, 0.5652, -0.2884), print_output=F)
uc.check(pol_=c(1, -0.4271), print_output=F)
...
## Diagnostic Checking
  # Select Models 1 and 3
model1 <- arima(train.bc, order=c(3, 2, 3), seasonal=list(order=c(1, 0, 1), 
                period=12), fixed=c(NA, NA, NA, 0, 0, NA, NA, NA), method="ML")
model3 <- arima(train.bc, order=c(2, 2, 4), seasonal=list(order=c(1, 0, 1), 
                period=12), fixed=c(NA, NA, NA, 0, NA, NA, NA, NA), method="ML")
  # Pull residuals
res1 <- model1$residuals
res3 <- model3$residuals
    ...
    # Model 1
index <- 1:length(res1)
plot(res1) # Plot Residuals
abline(h=mean(res1), col="red")
abline(lm(res1 ~ index), col="blue")
par(mfrow=c(1, 2)) # Histogram with normal curve and QQ-Plot
hist(res1)
m <- mean(res1)
sd <- sqrt(var(res1))
curve(dnorm(x, m, sd), add=TRUE)
qqnorm(res1)
qqline(res1)
par(mfrow=c(1, 2)) # ACF and PACF
acf(res1)
pacf(res1)
    # Portmanteau Tests
shapiro.test(res1) #Shapiro
Box.test(res1, lag=21, type=c("Box-Pierce"), fitdf=6) # Box-Pierce
Box.test(res1, lag=21, type=c("Ljung-Box"), fitdf=6) # Ljung-Box
Box.test(res1^2, lag=21, type=c("Ljung-Box"), fitdf=0) #McLeod-Li

    # Model 3
index <- 1:length(res3)
plot(res3) # Plot residuals
abline(h=mean(res3), col="red")
abline(lm(res3 ~ index), col="blue")
par(mfrow=c(1, 2)) # Histogram with normal curve and QQ-PLot
hist(res3)
m <- mean(res3)
sd <- sqrt(var(res3))
...
pacf(res3)
      # Portmanteau Tests
shapiro.test(res3) # Shapiro
Box.test(res3, lag=21, type=c("Box-Pierce"), fitdf=7) # Box-Pierce
Box.test(res3, lag=21, type=c("Ljung-Box"), fitdf=7) # Ljung-Box
...
U = U.tr^(1/lambda)
L = L.tr^(1/lambda)
plot.ts(m1.ts, xlim=c(1960, 2010), ylim=c(0, 3000))
lines(U, col="blue", lty="dashed")
lines(L, col="blue", lty="dashed")
points(seq(1960+length(train.bc)/12-1, 1960+length(train.bc)/12+1, 
           length.out=length(pred.tr$pred)), pred.orig, col="red")
    # Zoomed-in plot
plot.ts(m1.ts, xlim=c(1999, 2003), ylim=c(700, max(U)+400))
lines(U, col="blue", lty="dashed")
lines(L, col="blue", lty="dashed")
points(seq(1960+length(train.bc)/12-1, 1960+length(train.bc)/12+1, 
           length.out=length(pred.tr$pred)), pred.orig, col="red")
...
U = U.tr^(1/lambda)
L = L.tr^(1/lambda)
plot.ts(m1.ts, xlim=c(1960, 2025), ylim=c(0, 5000))
lines(U, col="blue", lty="dashed")
lines(L, col="blue", lty="dashed")
points(seq(1960+length(train.bc)/12-1, 1960+length(train.bc)/12+22, 
           length.out=length(pred.tr$pred)), pred.orig, col="red")
    # Zoomed-in plot
plot.ts(m1.ts, xlim=c(1999, 2025), ylim=c(700, max(U)+1500))
lines(U, col="blue", lty="dashed")
lines(L, col="blue", lty="dashed")
points(seq(1960+length(train.bc)/12-1, 1960+length(train.bc)/12+22, 
           length.out=length(pred.tr$pred)), pred.orig, col="red")
